...
function App() {
  ...
  const getClient = useCallback(async () => {
    const endpoint = await getHttpEndpoint({ network: IS_TESTNET ? 'testnet' : 'mainnet' });
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      if (newTokenData.imageFile) { imageUrl = await uploadToIPFS(newTokenData.imageFile); }
      const metadata = { name: newTokenData.name, symbol: newTokenData.symbol, description: newTokenData.description, image: imageUrl, decimals: 9 };
      ...
      await tonConnectUI.sendTransaction({
        ...
        messages: [{
          ...
          payload: beginCell().storeUint(0x12345678, 32).storeAddress(Address.parse(mockTokenAddress)).storeCoins(toNano(tokenAmount)).endCell().toBoc().toString('base64'),
        }],
      });
      setCreateStep(4);
      ...
      setIsCreating(false);
      setCreateStep(1);
      setNewTokenData({ name: '', symbol: '', description: '', image: '', marketCap: '25000', liquidity: '10000', launchPrice: '0.0001', imageFile: null });
      setActiveTab('home');
    } catch (err) {
  ...
  const executeApe = async () => {
    ...
    try {
      ...
      const tonAsset = Asset.native();
      const pool = client.open(await factory.getPool(PoolType.VOLATILE, [tonAsset, tokenAsset]));
      await tonConnectUI.sendTransaction({
        validUntil: Math.floor(Date.now() / 1000) + 600,
        messages: [{
          address: pool.address.toString(),
          amount: toNano(parseFloat(apeAmount)).toString(),
          payload: beginCell().storeUint(0x12345678, 32).storeAddress(Address.parse(walletAddress)).storeCoins(toNano(parseFloat(apeAmount))).endCell().toBoc().toString('base64'),
        }],
      });
      ...
      return (
        <div className="view-container creating">
          <div className="launch-animation">
            ...
            <p className="launch-sub">{createStep < 4 ? 'Do not close this window' : 'Redirecting to home...'}</p>
          </div>
        </div>
      );
    }
